<?php
  ...
  //BLEEDING $queuehtml = file_get_html($QUEUE_URL . '?api-key=' . $API_KEY, 0, $context);
  $queuehtml = file_get_html('http://dbo.chaseoes.com/api/bleeding/queue.html?api-key=' . $API_KEY, 0, $context);
  
  // PROJECTS
  $id = 0;
  foreach($queuehtml->find('table[id="projects"]') as $html) {
    foreach($queuehtml->find('td[class="col-project"] a[!target]') as $e) {
      $projects = $projects + 1;
    ...
    $num_claimed = 0;
    $id = 0;
    foreach($e->find('td[class="col-project"]') as $e) {
    ...
    $id = 0;
    foreach($queuehtml->find('td[class="col-category"] ul') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[id="projects"] td[class="col-user"]') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[id="projects"] td[class="col-count"] a') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[id="projects"] td[class="col-date"]') as $e) {
  ...
  foreach($queuehtml->find('table[id="files"] td[class="col-file-link"] a') as $e) {
    $uploaded_file_name = trim($e->innertext);
    array_push($file_listing, array('uploaded_file_name' => $uploaded_file_name));
  }
  
  $id = 0;
  foreach($queuehtml->find('table[id="files"] td[class="col-file"] a') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[id="files"] td[class="col-filesize"]') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[id="files"] td[class="col-file-link"]') as $e) {
    $status = 'ERROR';
    $claimed_by = 'null';
  ...
  $id = 0;
  foreach($queuehtml->find('table[id="files"] td[class="col-project"] a') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[id="files"] td[class="col-user"] a') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[id="files"] td[class="col-date"] span') as $e) {\
    date_default_timezone_set('UTC');
  ...
  $queuehtml = file_get_html($ADMIN_URL . 'reports/?api-key=' . $API_KEY, 0, $context);
  $id = 0;
  foreach($queuehtml->find('table[class="listing"] td[class="col-report"]') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[class="listing"] td[class="col-target"]') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[class="listing"] td[class="col-user"] a') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[class="listing"] td[class="col-report-category"]') as $e) {
  ...
  $id = 0;
  foreach($queuehtml->find('table[class="listing"] td[class="col-date] span') as $e) {\
    date_default_timezone_set('UTC');
    $uploaded_on = strtotime(trim(str_replace(" at", "", $e->plaintext)));
  ...
  //BLEEDING file_put_contents('/var/www/dbo/api/v1/cache.txt', $cache);
  file_put_contents('/var/www/dbo/api/bleeding/cache.txt', $cache);
  return;
?>